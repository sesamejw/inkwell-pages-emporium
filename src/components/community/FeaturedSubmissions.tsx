...
export const FeaturedSubmissions = () => {
  ...
  useEffect(() => {
    const fetchFeatured = async () => {
      try {
        // First get featured submissions
        ...
        if (!submissions || submissions.length === 0) {
          // Fallback to most liked submissions
          const { data: topSubmissions, error: topError } = await supabase
            .from('user_submissions')
            .select('id, title, description, content_type, image_url, user_id')
            .eq('status', 'approved')
          ...
          await enrichSubmissions(topSubmissions);
        } else {
  ...
  return (
    <div className="mb-8">
      <div className="flex items-center gap-2 mb-4">
        ...
        <h2 className="text-xl font-bold">Featured Creations</h2>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        {featured.map((submission) => {
          ...
          return (
            <Card
              key={submission.id}
              className="group cursor-pointer overflow-hidden border-2 border-amber-500/20 bg-gradient-to-b from-amber-500/5 to-transparent hover:border-amber-500/40 transition-all"
